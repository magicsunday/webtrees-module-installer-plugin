...
class ModuleInstaller extends LibraryInstaller
{
    ...
    public function getInstallPath(PackageInterface $package): string
    {
        ...
        return $webtreesPath
            . DIRECTORY_SEPARATOR . 'modules_v4'
            . DIRECTORY_SEPARATOR . $packageBaseName;
    }

    public function install(InstalledRepositoryInterface $repo, PackageInterface $package): ?PromiseInterface
    {
        // Skipping installation of the package, it's triggered by the
        // POST_PACKAGE_INSTALL and POST_PACKAGE_UPDATE events instead
        if ($this->skipInstall) {
            return resolve(null);
        }
    ...
    public function findWebtreesBasePath(): ?string
    {
        ...
        if ($this->composer->getPackage()->getName() === self::ROOT_PACKAGE_NAME) {
            return $this->getComposerBaseDir($this->composer->getConfig());
        }
